...
int_linelist <- NCoVUtils::get_international_linelist("Denmark")

# Get WHO sit rep case counts ---------------------------------------------

total_cases <- NCoVUtils::get_who_cases("Denmark", daily = TRUE)


# Join imported and local cases -------------------------------------------

cases <- EpiNow::get_local_import_case_counts(total_cases, int_linelist,
                                              cases_from = "2020-03-8")
...
rt_pipeline(
  cases = cases,
  linelist = linelist,
  target_folder = file.path("results/denmark", target_date),
