...
int_linelist <- NCoVUtils::get_international_linelist("Norway")

# Get WHO sit rep case counts ---------------------------------------------

total_cases <- NCoVUtils::get_who_cases("Norway", daily = TRUE)
...
rt_pipeline(
  cases = cases,
  linelist = linelist,
  target_folder = file.path("results/norway", target_date),
