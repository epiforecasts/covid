...
int_linelist <-  NCoVUtils::get_international_linelist("Iran") 

# Get WHO sit rep case counts ---------------------------------------------

total_cases <- NCoVUtils::get_who_cases("Iran", daily = TRUE)


# Join imported and local cases -------------------------------------------

cases <- EpiNow::get_local_import_case_counts(total_cases, int_linelist,
                                              cases_from = "2020-02-20")
...
rt_pipeline(
  cases = cases,
  linelist = linelist,
  target_folder = file.path("results/iran", target_date),
