...
local_linelist <- NCoVUtils::get_international_linelist("Czech Republic")

# Get WHO sit rep case counts ---------------------------------------------

total_cases <- NCoVUtils::get_who_cases("Czechia", daily = TRUE)

# Join imported and local cases -------------------------------------------

cases <- EpiNow::get_local_import_case_counts(total_cases, local_linelist,
...
rt_pipeline(
  cases = cases,
  linelist = linelist,
  target_folder = file.path("results/czechia", target_date),
