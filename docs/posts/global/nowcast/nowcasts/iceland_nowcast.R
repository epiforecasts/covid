...
int_linelist <- NCoVUtils::get_international_linelist("Iceland")

# Get WHO sit rep case counts ---------------------------------------------

total_cases <- NCoVUtils::get_who_cases("Iceland", daily = TRUE)
...
rt_pipeline(
  cases = cases,
  linelist = linelist,
  target_folder = "_posts/global/nowcast/results/iceland",
