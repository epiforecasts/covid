...
int_linelist <- NCoVUtils::get_international_linelist("Qatar")

# Get WHO sit rep case counts ---------------------------------------------

total_cases <- NCoVUtils::get_who_cases("Qatar", daily = TRUE)
...
rt_pipeline(
  cases = cases,
  linelist = linelist,
  target_folder = file.path("results/qatar", target_date),
