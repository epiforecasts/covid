...
data <- data %>%
  dplyr::rename(count = confirm) %>%
  dplyr::mutate(count = as.numeric(count)) %>%
  dplyr::group_by(source, region) %>%
  dplyr::mutate(count_ma = ma(as.numeric(count), 7)) %>%
  dplyr::ungroup()
...
plot_rt <- function(region_name) {
  ggplot(summary %>% dplyr::filter(region %in% region_name), 
	 aes(x = date, col = `Data source`, fill = `Data source`)) +
...
if (!interactive) {
  ...
  numeric_cols <-
    unname(which(apply(collated_table, 2,
                function(x) all(grepl("^[0-9e.() -]+$", x)))))
  sort_table <- apply(collated_table[, numeric_cols, with = FALSE], 2,
                      function(x) as.numeric(sub(" \\(.*\\)$", "", x)))
  sort_cols <- ncol(collated_table) + seq_len(ncol(sort_table))
  columnDefs <- 
