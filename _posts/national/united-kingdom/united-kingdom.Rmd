...
map <- uk_shp %>%
  ...
  ggplot2::ggplot(ggplot2::aes(fill = `Expected change in daily confirmed cases`)) +
  ggplot2::geom_sf(alpha = 0.8, size = 0.2)

map %>%
  EpiNow::theme_map(
    continuous = FALSE,
    variable_label = "Expected change in daily confirmed cases",
    fill_labels = ggplot2::waiver(),
    breaks = levels(summary_table$`Expected change in daily confirmed cases`)
  )
```
