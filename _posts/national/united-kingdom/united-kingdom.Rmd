...
We track transmission of Covid-19 in the United Kingdom using estimates of the time-varying reproduction number from a range of data sources (including test positive cases, hospital admissions, and death data) and spatial scales (national, regional (devolved authority and NHS region level in England), and local authority level). Where data is available these estimates are updated each day and published to a [publically available archive](https://dataverse.harvard.edu/dataverse/covid-rt) under an open source license. Estimates are also available from GitHub ([subnational estimates](https://github.com/epiforecasts/covid-rt-estimates/tree/master/subnational/united-kingdom/), [LTLA estimates](https://github.com/epiforecasts/covid-rt-estimates/blob/master/subnational/united-kingdom-local/cases/summary/rt.csv), [national estimates](https://github.com/epiforecasts/covid-rt-estimates/tree/master/national/cases/summary), and collated estimates](https://github.com/epiforecasts/covid-rt-estimates/blob/master/subnational/united-kingdom/collated/rt.csv)).

See our [Methods](https://epiforecasts.io/covid/methods) or our [paper](https://wellcomeopenresearch.org/articles/5-112/) for an explanation of how these estimates are derived, and our [pre-print](https://www.medrxiv.org/content/10.1101/2020.10.18.20214585v1) for methods comparing Rt estimates by data source
...
## Lower tier local authority level (LTLA)
...
```{r map-ltla,fig.height = 8, layout="l-body-outset", out.width = "70%", warning = FALSE, message = FALSE}
ltla_summary <- fread(here::here("covid-rt-estimates", "subnational", 
                                 "united-kingdom-local", "cases", 
                                 "summary", "summary_table.csv"))
uk_ltla_shp <- rnaturalearth::ne_states('United Kingdom', returnclass = 'sf') 
map <- uk_ltla_shp %>%
  mutate(region = name_en) %>% 
  left_join(ltla_summary %>%
  rename(region  = Region), by = "region") %>%
  ggplot(ggplot2::aes(fill = `Expected change in daily cases`)) +
  geom_sf(alpha = 0.8, size = 0.2)
map %>%
  theme_map(
    ...
    variable_label = "Expected change in daily cases",
    fill_labels = waiver(),
    breaks = levels(ltla_summary$`Expected change in daily cases`)
  )
```

*Figure1: The results of the latest reproduction number estimates (based on estimated confirmed cases with a date of infection in the United Kingdom, stratified by lower tier local authority, can be summarised by whether confirmed cases are likely increasing or decreasing. This represents the strength of the evidence that the reproduction number in each region is greater than or less than 1, respectively (see the [methods](https://epiforecasts.io/covid/methods.html) for details).*
