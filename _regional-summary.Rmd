...
`r if(regional_reports) '### Regional reports {.tabset .tabset-dropdown}'`


```{r generate-region-reports, include = FALSE, eval = regional_reports}
regions <- EpiNow::get_regions(file.path(path, "nowcast/results"))
...
region_summaries <- 1:length(regions) %>% 
  purrr::map(function(region_index) {
   ...
   out <- knitr::knit_child("_region-report.Rmd", envir = environment()) 
  })
```


```{r print-region-reports, results="asis", eval = regional_reports}
