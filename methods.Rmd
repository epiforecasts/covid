...
We used daily counts of confirmed cases reported by the European Centre for Disease Control for all analyses conducted at the national level [@ecdc_data; @NCoVUtils]. To estimate the delay from case onset date until confirmation date we used all cases from a publically available linelist for which onset and confirmation dates are available [@kraemer2020epidemiological; @NCoVUtils].

### Subnational data sources

For sub-national analyses, the source of the data is reported on each page, the data are fetched from government departments or from individuals who maintain a data source if no official data are available.
